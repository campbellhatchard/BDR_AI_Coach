const state = {
  ...
  difficulty: localStorage.getItem('ci_bdr_difficulty') || 'medium',
...
function renderTags(){
  ...
  [state.current.vertical, state.current.persona, 'Difficulty: ' + difficultyLabel(state.difficulty), state.current.label].forEach(t => {
...
function setDifficulty(){
  ...
  localStorage.setItem('ci_bdr_difficulty', state.difficulty);
  renderTags();
  if (state.current) {
    startScenario();
  }
...
els.difficultySelect.addEventListener('change', setDifficulty);
