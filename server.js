...
app.post('/api/respond', async (req, res) => {
  try {
    ...
    const { bdr_name, session_id, scenario, history, bdr_response } = req.body || {};
...
app.post('/api/log-summary', (req, res) => {
  const { session_id, bdr_name, scenario_label, summary_text, summary_score, summary_feedback } = req.body || {};
  appendEvent({
    type: 'summary',
    timestamp: Date.now(),
    session_id, bdr_name, scenario_label, summary_text, summary_score,
...
app.get('/api/manager-data', (req, res) => {
  ...
  for (const ev of events) {
    ...
    if (!byBdr[name]) byBdr[name] = { turns: 0, summaries: 0, scores: [], questionTypes: {}, weaknesses: {}, scenarios: {}, readiness: {} };
