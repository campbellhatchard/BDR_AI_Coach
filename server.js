...
app.get('/api/manager-data', (req, res) => {
  ...
  for (const ev of events) {
    ...
    if (!byBdr[name]) {
      byBdr[name] = {
        ...
        questionTypes: {},
        weaknesses: {},
        // scenarioSessions will map scenario -> set of session IDs to count sessions per scenario
        scenarioSessions: {},
        // sessionIds holds all unique session IDs for this rep
        sessionIds: new Set(),
        // sessionComplete marks sessions where the rep achieved Ready or Almost Ready and all discovery steps found
        sessionComplete: {},
        readiness: {},
        difficulties: {}
      };
    ...
    if (ev.type === 'turn') {
      row.turns += 1;
      if (typeof ev.score === 'number') row.scores.push(Number(ev.score));
      // accumulate question type counts
      for (const qt of (ev.question_type_detected || [])) row.questionTypes[qt] = (row.questionTypes[qt] || 0) + 1;
      // accumulate weakness counts
      ...
      // track scenario sessions (count sessions per scenario)
      const scen = ev.scenario_label || 'Unknown';
      if (!row.scenarioSessions[scen]) row.scenarioSessions[scen] = new Set();
      if (sid) row.scenarioSessions[scen].add(sid);
      // accumulate readiness counts
      const ready = ev.transition_readiness || 'not_ready';
      row.readiness[ready] = (row.readiness[ready] || 0) + 1;
      // accumulate difficulties
      const diff = ev.difficulty || 'medium';
      row.difficulties[diff] = (row.difficulties[diff] || 0) + 1;
      // check if this turn constitutes a completed session (ready/almost_ready and all discovery steps found)
      let complete = (ready === 'ready' || ready === 'almost_ready');
      if (complete) {
        const dp = ev.discovery_progress || {};
        const keys = ['problem_location','likely_cause','business_impact','why_it_matters_now'];
        for (const k of keys) {
          if (dp[k] !== 'found') { complete = false; break; }
        }
      }
      if (complete && sid) {
        row.sessionComplete[sid] = true;
      }
  ...
  const rows = Object.entries(byBdr).map(([bdr_name, row]) => {
    ...
    // compute practice metrics
    const distinctScenarios = Object.keys(row.scenarioSessions).length;
    const sessions = row.sessionIds.size;
    const completions = Object.keys(row.sessionComplete).length;
    const practiceVolume = distinctScenarios + sessions + completions;
    // build scenarios array sorted by number of sessions per scenario
    const scenariosList = Object.entries(row.scenarioSessions).map(([scen, set]) => [scen, set.size]).sort((a,b) => b[1] - a[1]).slice(0,5);
    return {
      bdr_name,
      turns: row.turns,
      ...
      practice: {
        distinct_scenarios: distinctScenarios,
        sessions: sessions,
...
app.get('/api/bdr/:name', (req, res) => {
  ...
  const readiness = {};
  const questionTypes = {};
  // scenarioSessions will map scenario -> set of session IDs
  const scenarioSessions = {};
  const strengths = {};
  const weaknesses = {};
  // track unique sessions and completion status
  ...
  for (const ev of events) {
    if (ev.type === 'turn') {
      ...
      const scen = ev.scenario_label || 'Unknown';
      const sid = ev.session_id || null;
      // track scenario session sets
      if (!scenarioSessions[scen]) scenarioSessions[scen] = new Set();
      if (sid) scenarioSessions[scen].add(sid);
  ...
  // compute practice metrics
  const distinctScenarios = Object.keys(scenarioSessions).length;
  const sessions = sessionIds.size;
  const completions = Object.keys(sessionComplete).length;
  const practiceVolume = distinctScenarios + sessions + completions;
  ...
  const resp = {
    ...
    scenarios: Object.entries(scenarioSessions).map(([scn, set]) => [scn, set.size]).sort((a,b)=>b[1]-a[1]),
